...
Консервативный режим для маленького банка (`$100`) включается через `--risk-profile bankroll_100`. Он специально делает меньше сделок: выше edge threshold, меньше max stake, жестче лимиты по event/city/date, ниже допустимый spread.
...
Для live-like paper перед Strategy Lab можно забрать read-only snapshot стаканов CLOB. Это не выставляет ордера:
...
Для банка `$100` предпочтительный запуск:
...
Текущий `bankroll_100` snapshot после CLOB depth-check:

- `859` market rows;
- `133` trade-сигнала после первичных фильтров;
- `9` robust candidates после stress + execution checks;
- `8` selected paper positions;
- stake: `$4.00` из `$100`;
- all selected positions depth-fillable in current snapshot: `8 / 8`;
- max city exposure: `$1.00`;
- max date exposure: `$2.00`.
